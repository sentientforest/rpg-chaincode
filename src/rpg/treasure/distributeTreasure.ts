...
import { CharacterEntity, PartyEntity, PartyTreasury, TreasureDistribution } from "../types";
...
export async function distributeTreasure(ctx: GalaChainContext, dto: DistributeTreasureDto): Promise<void> {
  ...
  if (dto.partyId) {
    const partyKey = PartyEntity.getCompositeKeyFromParts(PartyEntity.INDEX_KEY, [dto.partyId]);
    ...
    const treasuryKey = PartyTreasury.getCompositeKeyFromParts(PartyTreasury.INDEX_KEY, [dto.partyId]);
