...
import { CharacterEntity, CharacterEquipment, EquipItemDto } from "../types";

export async function equipItem(ctx: GalaChainContext, dto: EquipItemDto): Promise<void> {
  ...
  const equipmentKey = CharacterEquipment.getCompositeKeyFromParts(CharacterEquipment.INDEX_KEY, [
    dto.characterName,
    dto.itemId
  ]);
