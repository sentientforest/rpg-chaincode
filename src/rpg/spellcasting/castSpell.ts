...
export async function castSpell(ctx: GalaChainContext, dto: CastSpellDto): Promise<CastSpellAction> {
  ...
  const knownSpell = characterSpells.find(
    (spell) => spell.spellName === dto.spellName && spell.tradition === dto.tradition
  );
  ...
  if (dto.castAtLevel < spellData.level) {
    throw new Error(
      `Cannot cast ${dto.spellName} at level ${dto.castAtLevel}, minimum level is ${spellData.level}`
    );
  }
  ...
  if (dto.castingSource === "prepared" || dto.castingSource === "spontaneous") {
    ...
    // Get the spell slot
    const slotKey = SpellSlot.getCompositeKeyFromParts(SpellSlot.INDEX_KEY, [
      dto.characterName,
      dto.castAtLevel,
      dto.spellSlotUsed
    ]);
    ...
    try {
      ...
      if (spellSlot.tradition !== dto.tradition) {
        throw new Error(
          `Spell slot tradition ${spellSlot.tradition} does not match spell tradition ${dto.tradition}`
        );
      }
